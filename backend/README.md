...
2. **Importação**: Os leads são lidos de um arquivo `.csv` (separado por `,` ou `;`) pelo endpoint `/upload-leads-file` ou puxados automaticamente de `/dados/leads.csv` pelo endpoint `/import-local-leads`.
3. **Inteligência de Importação**: O sistema identifica o número, remove caracteres especiais e adiciona o código do Brasil (`55`). Contatos já existentes têm seu `status` atualizado para ficar em sincronia com a planilha.
4. **Disparo**: O endpoint `/send` varre todos os leads com status diferente de `"enviado"`, processa o texto dinâmico (trocando a tag `{name}`) e envia (seja texto ou mídia).
5. **Sincronização**: Ao final do disparo, o banco de dados é atualizado e o arquivo físico (`dados/leads.csv`) é reescrito com o status final de cada lead (`enviado`, `falhou` ou `pendente`).
